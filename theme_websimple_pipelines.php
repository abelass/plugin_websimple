...
function theme_websimple_formulaire_charger($flux) {
	...
	if ($form == 'inscription_client' and _request('page') == 'selection') {
...
function theme_websimple_formulaire_traiter($flux) {
	...
	$form = $flux['args']['form'];
	if (($form == 'editer_client' or $form == 'inscription_client') and _request('page') == 'selection' and include_spip('inc/paniers') and $id_panier = paniers_id_panier_encours()) {
		...
		if (!$id_auteur) {
			...
			editer_commande_details($id_commande, array(
				'id_auteur' => $id_auteur
			));
		}

		// On récupère le contenu du panier
		$panier = sql_allfetsel('*', 'spip_paniers_liens', 'id_panier = ' . $id_panier);
		...
		if ($panier and is_array($panier)) {
			...
			foreach ($panier as $cle => $emplette) {
				...
				sql_insertq('spip_commandes_details', array(
		...
		if ($form = 'inscription_client')
			$flux['data']['retour_inscription'] = 'ok';
		('websimple:commande_enregistre');
	}
	return ($flux);
}
